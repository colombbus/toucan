...
class EvaluationState_Model extends ORM {

    protected $table_name = "evaluation_states";
    protected $ignored_columns = array('translatedName');

    const UNDER_CONSTRUCTION = 1;
    const GOING_ON = 2;
    const UNDER_ANALYSE = 3;
    const OVER = 4;
    const CANCELLED = 5;

    public function getTranslatedName() {
        if ($this->loaded) {
            return Kohana::lang('evaluation.'.$this->name);
        }
        return "";
    }

    public static function getTranslatedStates() {
        $states = self::factory('evaluationState');
