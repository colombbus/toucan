...
class Indicator_Model extends Toucan_Model implements Ajax_Model {
    ...
    public function duplicate(& $user, $categoryId = null, $next = false) {
        ...
        if (isset($categoryId)) {
            ...
            $constraints = array();
            if ($next) {
                $categoryOrder = $category->getIndicatorOrder($this->id);
    ...
    public function register(& $category, $value, $save, $constraints = array()) {
        ...
        if ($value) {
            ...
            if (!in_array($category->id, $categories)) {
                ...
                if ($save) {
                    $this->save();
                    $category->updateIndicatorOrders($constraints);
                }
            }
        } else {
            // unregister
    ...
    public function getCategoriesEditableData(& $user) {
        $editableData = array();
        $categories = $this->generic_parent->getCategories($user, false);
    ...
    public function validateCategorySelection(& $array, $save = false) {
        ...
        foreach ($this->categories as $category) {
            $category->updateIndicatorOrders();
        }
        if ($save)
            $this->save();
        return true;
